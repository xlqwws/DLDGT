...
from utilities import get_identifier, load_feather_plus_metadata, save_feather_plus_metadata
...
class DGTTransformer:
    ...
    def build_model(self, prefix:str=None):
        ...
        for categorical_feature_name, categorical_feature_levels in \
            ...
            m_input = Input(
                (self.parameters.window_size, 1 if self.model_input_spec.categorical_format == ClassificationFormat.Integers else categorical_feature_levels),
                name=f"{prefix}input_{categorical_feature_name}",
                dtype="int32" if self.model_input_spec.categorical_format == ClassificationFormat.Integers else "float32"
            )
            m_inputs.append(m_input)
    ...
    def _load_preprocessed_dataset(self, dataset_name:str,
        ...
        if dataset_name is None:
            raise Exception(f"Dataset name must be specified so NetworkTransformer can optimise operations between subsequent calls!")
        ...
        if DGTTransformer.retain_inmem_cache:
            if DGTTransformer.inmem_cache is not None and cache_key in DGTTransformer.inmem_cache:
                print(f"Using in-memory cached version of this pre-processed dataset. To turn off this functionality set NetworkTransformer.retain_inmem_cache = False")
                return DGTTransformer.inmem_cache[cache_key]
        ...
        for i, col_name in enumerate(categorical_columns):
            ...
            if self.input_encoding.required_input_format == ClassificationFormat.OneHot:
        ...
        if DGTTransformer.retain_inmem_cache:
            if DGTTransformer.inmem_cache is None:
                DGTTransformer.inmem_cache = {}

            DGTTransformer.inmem_cache.clear()
            DGTTransformer.inmem_cache[cache_key] = (new_df, model_input_spec)
    ...
    def evaluate(self, m:keras.Model, batch_size, early_stopping_patience:int=5, epochs:int=100, steps_per_epoch:int=128):
        ...
        def samplewise_to_featurewise(X):
            ...
            if len(feature_columns_map) == 0:
                for feature in self.model_input_spec.feature_names:
                    if feature in self.model_input_spec.numeric_feature_names or self.model_input_spec.categorical_format == ClassificationFormat.Integers:
        ...
        def run_evaluation(epoch):
            ...
            print(f"F1 Score: {f1_score * 100:.2f}%, Balanced Accuracy: {balanced_accuracy * 100:.2f}%")
            print(f"Miss Rate: {miss_rate * 100:.2f}%, False Alarm Rate: {false_alarm_rate * 100:.2f}%")
            print(f"TP = {TP:,} / {n_eval_P:,}, TN = {TN:,} / {n_eval_N:,}, FP = {FP:,}, FN = {FN:,}")
